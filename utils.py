...
async def convert_chunk_to_wav(data: bytes, suffix=None, websocket: WebSocket = None, 
    ...
    if save_chunks:
        ...
        # Translate the audio using the saved file
        if translate_func:
        ...
        print(f"Processing audio without saving to disk")
        
        # Create a temporary file for translation if needed
        if translate_func:
            with open(filename, 'wb') as f:
                f.write(wav_data)
...
async def send_audio_to_client(websocket: WebSocket, audio_path: str = None, buffer: io.BytesIO = None):
    """
    ...
    Args:
        ...
        buffer: In-memory buffer containing audio data (alternative to audio_path)
    """
    if not websocket:
        return
        
    try:
        # Get audio data either from file or buffer
        if audio_path:
